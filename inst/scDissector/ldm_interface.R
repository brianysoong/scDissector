...
select_cells = function(
  ...
  if (is.null(samples)) {
    ...
    return(cells[
      session$userData$dataset$cell_to_cluster[cells] %in%
        clusters &
        session$userData$dataset$cell_to_sample[cells] %in% samples
    ])
  }
}
...
get_counts_array = function(
  ...
  if (is.null(clusters)) {
    ...
    clusters = intersect(
      clusters,
      dimnames(session$userData$dataset$counts)[[3]]
    )
  }
  session$userData$dataset$counts[samples, genes, clusters, drop = F]
}

get_noise_counts_array = function(
  session,
  samples = NULL,
  genes = NULL,
  clusters = NULL
  ...
  if (is.null(samples)) {
    ...
    samples = intersect(
      samples,
      dimnames(session$userData$dataset$noise_counts)[[1]]
    )
  }
  if (is.null(genes)) {
    ...
    genes = intersect(
      genes,
      dimnames(session$userData$dataset$noise_counts)[[2]]
    )
  }
  if (is.null(clusters)) {
    ...
    clusters = intersect(
      clusters,
      dimnames(session$userData$dataset$noise_counts)[[3]]
    )
  }
