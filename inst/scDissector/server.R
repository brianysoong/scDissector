...
update_clusters = function(session, new_clusts, delete_old = F) {
  ...
  updateTextInput(
    session,
    "inClusters",
    value = paste(new_clusts, collapse = ",")
  )
}
...
update_all = function(session, ldm) {
  ...
  session$userData$reactiveVars$clusters_genes_samples_reactive = list(
    clusters = c(),
    genes = c(),
    samples = c()
  )
  session$userData$reactiveVars$clusters_samples_reactive = list(
    clusters = c(),
    samples = c()
  )
  session$userData$sample_colors = default_sample_colors[
    1:length(get_loaded_samples(session))
  ]
  ncells_choices = as.numeric(setdiff(
    params$nrandom_cells_per_sample_choices,
    "All"
  ))
  ncells_per_sample = ncells_choices[which.min(abs(
    (2000 / length(get_loaded_samples(session))) - ncells_choices
  ))]
  ...
  clust_title = paste(
    session$userData$cluster_order,
    " - ",
    annots[session$userData$cluster_order],
    sep = ""
  )
  print(clust_title)
  ...
  updateTextInput(
    session,
    "inSamplesToShow",
    value = paste(get_loaded_samples(session), collapse = ", ")
  )
  updateSelectInput(
    session,
    "inClustForDiffGeneExprsProjVsRef",
    choices = clust_title
  )
  cluster_sets = unique(unlist(get_nodes(as_cluster_sets_recursive(
    session$userData$cluster_sets
  ))))
  updateSelectInput(
    session,
    "categorizeSamplesBy",
    choices = colnames(session$userData$sample_annots),
    selected = "sample_ID"
  )
  updateSelectInput(
    session,
    "removeClusterSetSelectInput",
    choices = cluster_sets
  )
  updateSelectInput(
    session,
    "inReorderSamplesBy",
    choices = c("All", cluster_sets)
  )
  updateSelectInput(
    session,
    "inGatingSample",
    choices = get_loaded_samples(session)
  )
  updateSelectInput(session, "inGatingShowClusters", choices = clust_title)
  updateSelectInput(
    session,
    "input$inTruthNcellsPer",
    choices = params$nrandom_cells_per_sample_choices,
    selected = ncells_per_sample
  )
  updateSelectInput(
    session,
    "inQCDownSamplingVersion",
    choices = get_ds_options(session),
    selected = ifelse(
      "1000" %in% get_ds_options(session),
      "1000",
      max(get_ds_options(session))
    )
  )
  updateSelectInput(
    session,
    "inTruthDownSamplingVersion",
    choices = get_ds_options(session),
    selected = ifelse(
      "1000" %in% get_ds_options(session),
      "1000",
  ...
  updateSelectInput(
    session,
    "inModulesDownSamplingVersion",
    choices = get_ds_options(session),
    selected = max(get_ds_options(session))
  )
  updateTree(
    session,
    "clusters_sets_shinytree",
    data = as_list_recursive(ldm$cluster_sets)
  )
  if (is.null(get_noise_models(session))) {
    updateSelectInput(
      session,
      "inModelOrAverage",
      choices = c("Model parameters", "Average")
    )
  }
...
load_metadata = function(session, datapath) {
  ...
  if (read_flag) {
    session$userData$vers_tab <- read.csv(
      file = verfile,
      header = T,
      stringsAsFactors = F
    )
    session$userData$samples_tab <- read.csv(
      file = samples_file,
      header = T,
      stringsAsFactors = F
    )
  ...
  if (file.exists(myGeneListFile)) {
    added_gene_list_tmp = read.delim(
      file = myGeneListFile,
      header = T,
      stringsAsFactors = F
    )
    ...
    if (
      length(added_gene_list_tmp[, 1]) !=
        length(unique(added_gene_list_tmp[, 1]))
    ) {
      message(
        myGeneListFile,
        " was not loaded since gene sets names are not unique."
      )
    } else {
      names2 = c(added_gene_list_tmp[, 1], names(session$userData$geneList))
      session$userData$geneList <- c(
        added_gene_list_tmp[, 2],
        session$userData$geneList
      )
  ...
  if (dir.exists(paste(datapath, "metadata", sep = "/"))) {
    if (file.exists(sample_annots_fn)) {
      session$userData$sample_annots = read.csv(
        sample_annots_fn,
        stringsAsFactors = F
      )
      rownames(
        session$userData$sample_annots
      ) = session$userData$sample_annots$sample_ID
  ...
  if (file.exists(mySampleSetsFile)) {
    sample_sets_tab <- read.table(
      file = mySampleSetsFile,
      header = T,
      stringsAsFactors = F,
      row.names = 1
    )
    session$userData$sample_sets <- strsplit(
      sample_sets_tab[, "samples"],
      ",| ,|, "
    )
    names(session$userData$sample_sets) <- rownames(sample_sets_tab)
    session$userData$samples_to_show = c(
      names(session$userData$sample_sets),
      samples_to_show
    )
  } else {
...
function(input, output, session) {
  ...
  observeEvent(input$inDatapath, {
    ...
    updateSelectInput(
      session,
      "inGeneSets",
      choices = names(session$userData$geneList)
    )
    updateSelectInput(
      session,
      "inModelVer",
      "Model Version:",
      choices = session$userData$vers_tab$title
    )
    updateSelectInput(
      session,
      "inSampleToAdd",
      "Samples:",
      choices = session$userData$samples_to_show
    )
  })
  ...
  observeEvent(input$inLoad, {
    ...
    model_fn = paste(
      input$inDatapath,
      session$userData$vers_tab$path[ifelse(is.na(i), 1, i)],
      sep = "/"
    )
    ...
    if (!all(samples %in% session$userData$samples_tab$index)) {
      browser()
      message(
        "Error: These samples could not be recognized: ",
        paste(
          samples[!(samples %in% session$userData$samples_tab$index)],
          collapse = ","
        ),
        ". Loading failed!"
      )
    ...
    sample_paths = paste(
      input$inDatapath,
      session$userData$samples_tab$path[match(
        samples,
        session$userData$samples_tab$index
      )],
      sep = "/"
    )
    ...
    ldm = scDissector::load_dataset_and_model(
      model_fn,
      sample_paths,
      min_umis = min_umis,
      max_umis = max_umis,
  ...
  observeEvent(input$inSamplesAddClusterSet, {
    ...
    updateTextAreaInput(
      session,
      "inSamplesClusterSets",
      value = paste(unique(c(cluster_sets, added)), collapse = ", ")
    )
  })
  ...
  observeEvent(input$inReorderSamples, {
    ...
    freq_norm = normalize_by_clusterset_frequency(
      get_cell_to_cluster(session),
      get_cell_to_sample(session),
      insamples,
      cluster_sets,
      pool_subtype = T,
      reg = 0
    ...
    if (nrow(x) >= 2) {
      ...
      updateTextInput(
        session,
        "inSamplesToShow",
        value = paste(insamples[ord], collapse = ", ")
      )
    }
  })
  ...
  observeEvent(input$inAddSample, {
    s1 = paste(
      unique(c(strsplit(input$inSamples, ",|, | ,")[[1]], input$inSampleToAdd)),
      collapse = ", "
    )
  ...
  observeEvent(input$inAbsOrRel, {
    if (input$inAbsOrRel == "Absolute") {
      ...
      updateSliderInput(
        ...
        max = -.5,
        step = .5,
        value = session$userData$prev_inModelColorScale_abs
      )
      ...
      updateSliderInput(
        session,
        "inModelColorScale",
        label = "Log2(expression/mean)",
        min = -8,
        max = 8,
        step = 1,
        value = session$userData$prev_inModelColorScale_rel
      )
    }
  })
  ...
  observe({
    ...
    clusts = intersect(
      clusts,
      setdiff(
        get_all_clusters(session),
        session$userData$scDissector_params$excluded_clusters
      )
    )
    ...
    if (
      (!is.null(xy_range)) &
        (sum(c('xaxis.range[0]', 'xaxis.range[1]') %in% names(xy_range)) == 2)
    ) {
      if (!all(session$userData$prev_xy_range_G == unlist(xy_range))) {
        if (min(unlist(xy_range), na.rm = T) < 0) {
          ...
          session$userData$prev_xy_range_G = unlist(xy_range)
          genes = genes[
            ceiling(as.numeric(xy_range$'xaxis.range[0]')):floor(as.numeric(
              xy_range$'xaxis.range[1]'
            ))
          ]
    ...
    if (
      (!is.null(xy_range)) &
        (sum(c('yaxis.range[0]', 'yaxis.range[1]') %in% names(xy_range)) == 2)
    ) {
      if (!all(session$userData$prev_xy_range_C == unlist(xy_range))) {
        if (min(unlist(xy_range), na.rm = T) < 0) {
          ...
          session$userData$prev_xy_range_C = unlist(xy_range)
          clusts = rev(rev(clusts)[
            ceiling(as.numeric(xy_range$'yaxis.range[0]')):floor(as.numeric(
              xy_range$'yaxis.range[1]'
            ))
          ])
        }
  ...
  vis_freqs_cluster_sets_reactive <- reactive({
    return(
      cluster_sets = unique(strsplit(input$inSamplesClusterSets, ",|, | ,")[[
        1
      ]])
    )
  })
  ...
  cluster_annots_reactive <- reactive({
    ...
    if (is.null(session$userData$clustAnnots) && (is.null(cluster_sets))) {
      ...
      if (is.null(cluster_sets)) {
        ...
        edges = edges[
          match(session$userData$default_clusters, edges[, "node"]),
        ]
        annots = edges[, 2]
        names(annots) = edges[, 1]
        return(annots)
  ...
  sample_annots_reactive <- reactive({
    return(session$userData$sample_annots[
      intersect(
        get_loaded_samples(session),
        rownames(session$userData$sample_annots)
      ),
    ])
  })
  ...
  sample_cells_by_cluster_reactive <- reactive({
    ...
    if (
      is.na(match(
        nrandom_cells,
        session$userData$randomly_selected_cells_by_cluster[[ds_i]]
      ))
    ) {
      session$userData$randomly_selected_cells_by_cluster[[ds_i]][[
        nrandom_cells
    ...
    for (clusteri in unlist(session$userData$cluster_sets)) {
      maski = select_cells(
        session,
        cells = get_ds_cells(session, ds_i),
        clusters = clusteri
      )
      if (length(maski) == 0) {
        next
      ...
      if (
        nrandom_cells == "All" ||
          pmax(0, as.numeric(nrandom_cells), na.rm = T) >= length(maski)
      ) {
        session$userData$randomly_selected_cells_by_cluster[[ds_i]][[
          nrandom_cells
        ]] <- c(
        ...
        session$userData$randomly_selected_cells_by_cluster[[ds_i]][[
          ...
          sample(maski, size = as.numeric(nrandom_cells), replace = F)
        )
      }
    }
  ...
  common_sample_cells_func = function(downsampling_version, ncells_per_sample) {
    ...
    if (
      is.na(match(
        ncells_per_sample,
        session$userData$randomly_selected_cells_by_sample[[ds_i]]
      ))
    ) {
      session$userData$randomly_selected_cells_by_sample[[ds_i]][[
        ncells_per_sample
    ...
    for (sampi in get_loaded_samples(session)) {
      ...
      if (
        ncells_per_sample == "All" ||
          pmax(0, as.numeric(ncells_per_sample), na.rm = T) >= length(maski)
      ) {
        session$userData$randomly_selected_cells_by_sample[[ds_i]][[
          ncells_per_sample
        ]] <- c(
        ...
        session$userData$randomly_selected_cells_by_sample[[ds_i]][[
          ...
          sample(maski, size = as.numeric(ncells_per_sample), replace = F)
        )
      }
    }
  ...
  sample_cells_by_sample_reactive <- reactive({
    common_sample_cells_func(
      input$inTruthDownSamplingVersion,
      input$inTruthNcellsPer
    )
  })
  ...
  sample_cells_by_sample_reactive_QC <- reactive({
    common_sample_cells_func(
      input$inQCDownSamplingVersion,
      input$inQCNcellsPerSample
    )
  })
  ...
  ds_cells_QC_reactive <- reactive({
    ...
    sampling_mask = sample_cells_by_sample_reactive_QC()[[match(
      input$inQCDownSamplingVersion,
      get_ds_options(session)
    )]][[input$inQCNcellsPerSample]]
    return(select_cells(
      session,
      cells = sampling_mask,
  ...
  observeEvent(input$inGeneSets, {
    ...
    updateTextInput(
      session,
      "inGenes",
      value = as.character(session$userData$geneList[geneSets])
    )
  })
  ...
  observeEvent(input$inClusterModules, {
    ...
    if (input$inReorderingModulesMethod == "Hierarchical clustering") {
      ...
      updateTextInput(
        session,
        "inModules",
        value = paste(rownames(cormat)[ord], collapse = ",")
      )
      ...
      updateTextInput(
        session,
        "inModules",
        value = paste(rownames(modulemat)[ord], collapse = ",")
      )
    }
  })
  ...
  observeEvent(input$inModulesAddGeneList, {
    ...
    genes_to_show_comma_delimited = paste(
      unlist(modsl[module_ids]),
      collapse = ", "
    )
    new_set_name = paste(
      "Modules_",
      input$inNUmberOfGeneModules,
      "_",
      date(),
      sep = ""
    )
    ...
    updateSelectInput(
      ...
      "inGeneSets",
      choices = names(session$userData$geneList),
      selected = names(session$userData$geneList)[length(
        session$userData$geneList
      )]
    )
  })
  ...
  observeEvent(input$inResetModules, {
    updateTextInput(
      session,
      "inModules",
      value = paste(names(gene_to_module_reactive()), collapse = ",")
    )
  })
  ...
  observeEvent(input$inVarMeanScreenSelectedClusters, {
    ...
    mask1 = s1 / (numis * ncol(ds)) >
      10^as.numeric(input$inMinExprForScreen[1]) &
      s1 / (numis * ncol(ds)) < 10^as.numeric(input$inMinExprForScreen[2])
    ...
    if (ngenes_to_show > length(inclusts)) {
      positive_var_gens = names(var_score)[
        pmax(var_score, 0, na.rm = T) > 0.001
      ]
      umisum = as.matrix(aggregate.Matrix(
        t(ds[positive_var_gens, ]),
        get_cell_to_cluster(session, cells = colnames(ds))
      ))
      cluster_avg = (t(umisum / rowSums(umisum)))
      cluster_avg_normed = log2(
        (1e-6 + cluster_avg) / (1e-6 + rowMeans(cluster_avg))
      )
      high_var_genes = as.vector(unlist(sapply(
        split(
          rownames(cluster_avg_normed),
          apply(cluster_avg_normed, 1, which.max)
        ),
        FUN = function(x) {
          names(head(
            sort(var_score[x], decreasing = T),
            floor(ngenes_to_show / ncol(cluster_avg_normed))
          ))
        }
      )))
    } else {
      high_var_genes = names(head(
        sort(var_score, decreasing = T),
        ngenes_to_show
      ))
    }
    #
  ...
  observeEvent(input$inBlindChisqSelectedClusters, {
    ...
    counts_to_chisq = get_counts_array(
      session,
      samples = samples,
      genes = genes,
      clusters = clusters
    )
    noise_counts = get_noise_counts_array(
      session,
      samples = samples,
      genes = genes,
      clusters = clusters
    ...
    mask = rownames(chisq_res2) %in%
      genes &
      chisq_res2[, 3] < 0.01 &
      rowSums(
        get_umitab(session, cells = cells, genes = rownames(chisq_res2)) > 1
      ) >=
        10
    isolate({
      ngenes_to_show = as.numeric(input$inNgenes)
    ...
    a = chisq_res2[mask, ]
    genes_to_show = head(
      rownames(a)[order(a[, 2], decreasing = T)],
      ngenes_to_show
    )
  ...
  observeEvent(input$inAddClusterSetButton, {
    ...
    updateSelectInput(
      session,
      "removeClusterSetSelectInput",
      choices = unique(get_nodes(l))
    )
    updateSelectInput(
      session,
      "inReorderSamplesBy",
      choices = c("All", unique(get_nodes(l)))
    )
  })
  ...
  observeEvent(input$inRemoveClusterSetButton, {
    ...
    updateSelectInput(
      session,
      "removeClusterSetSelectInput",
      choices = unique(unlist(get_nodes(l2)))
    )
    updateSelectInput(
      session,
      "inReorderSamplesBy",
      choices = c("All", unique(unlist(get_nodes(l2))))
    )
  })
  ...
  observeEvent(input$saveClusterSetButtion, {
    ...
    #Bug fix AL 08/09/19
    cluster_set_fn = paste(
      substr(
        session$userData$loaded_model_file,
        1,
        nchar(session$userData$loaded_model_file) - 3
      ),
      "_cluster_sets.txt",
      sep = ""
    )
  ...
  chisq_genes = function(counts) {
    ...
    arrcont = array(
      c(
        counts,
        matrix(cluster_tot, dim(counts)[1], dim(counts)[2], byrow = T) - counts
      ),
      dim = c(dim(counts), 2)
    )
    suppressWarnings({
      res = t(apply(arrcont, 1, function(x) {
        unlist(chisq.test(x)[c("p.value", "statistic")])
      }))
    })
  ...
  observeEvent(input$inFC, {
    ...
    genes_to_show = head(
      names(sort(fc, decreasing = T)),
      floor(ngenes_to_show / 2)
    )
    genes_to_show = c(
      genes_to_show,
      tail(names(sort(fc, decreasing = T)), ceiling(ngenes_to_show / 2))
    )
    genes_to_show_comma_delimited = paste(genes_to_show, collapse = ",")
    ...
    names(geneList)[length(geneList)] = paste(
      "FC_",
      pref,
      "_",
      ngenes_to_show,
      "_",
      paste(clusts_fg, collapse = "_"),
      "_VS_",
      paste(clusts_bg, collapse = "_"),
      sep = ""
    )
  ...
  observeEvent(input$inResetClusters, {
    clust_title = paste(
      session$userData$default_clusters,
      " - ",
      cluster_annots_reactive()[session$userData$default_clusters],
      sep = ""
    )
  ...
  observeEvent(input$inSaveClusterOrder, {
    ...
    order_fn = paste(
      strsplit(session$userData$loaded_model_file, "\\.")[[1]][1],
      "_order.txt",
      sep = ""
    )
  ...
  observeEvent(input$inOrderClusters, {
    ...
    clust_title = paste(
      inclusts,
      " - ",
      cluster_annots_reactive()[inclusts],
      sep = ""
    )
    ...
    updateSelectInput(
      session,
      "inAnnotateClusterNum",
      choices = clust_title[reorderv1]
    )
  })
  ...
  modules_varmean_reactive = reactive({
    ...
    genemask = ds_mean > 10^input$inVarMeanXlim[1] &
      ds_mean < 10^input$inVarMeanXlim[2]
    ds = ds[genemask, ]
  ...
  getGeneModuleMask = function() {
    ...
    isolate({
      geneModuleMask <- log10(df$m) > as.numeric(input$inVarMean_MeanThresh) &
        log2(df$v / df$m) > lline + as.numeric(input$inVarMean_varmeanThresh)
    })
  ...
  module_cor_reactive = reactive({
    ...
    cormat = get_avg_gene_to_gene_cor(
      ds[names(which(getGeneModuleMask())), ],
      get_cell_to_sample(session, cells = colnames(ds))
    )
    return(cormat)
  })
  ...
  gene_to_module_reactive = reactive({
    ...
    c2c = cutree(
      ...
      k = as.numeric(input$inNUmberOfGeneModules)
    )
    modsl = split(names(c2c), c2c)
    updateTextInput(
      session,
      "inModules",
      value = paste(names(modsl), collapse = ",")
    )
    updateSelectInput(
      session,
      "inModuleSelect",
  ...
  output$inDownloadModuleList <- downloadHandler(
    ...
    content = function(file) {
      ...
      tab = cbind(
        module_name = names(modsl),
        genes = sapply(modsl, paste, collapse = ",")
      )
  ...
  output$mytable = DT::renderDataTable({
    ...
    DT::datatable(
      ...
      rownames = F,
      escape = F,
      selection = list(mode = 'multiple', selected = 1:nrow(tab))
    )
  })
  ...
  output$UMI_boxplot <- renderPlot({
    ...
    boxplot(
      split(log10(colSums(u)), get_cell_to_cluster(session, cells = cells))[
        clusters
      ],
      las = 2,
      ylab = "log10(#UMIs)"
  ...
  output$gating_plots <- renderPlot({
    ...
    cell_to_cluster = get_cell_to_cluster(
      session,
      cells = select_cells(session, samples = samp)
    )
    ...
    for (i in 1:nplots) {
      ...
      plot(
        numis[mask],
        insilico_gating_scores[[i]][mask],
        log = "x",
        ylab = paste("fraction", names(clustering_params$insilico_gating)[i]),
        xlab = "#UMIs",
        col = ifelse(
          is.na(cell_to_cluster[mask]),
          "gray",
          ifelse(cell_to_cluster[mask] == clust, 2, 1)
        )
      )
      points(
        numis[mask],
        insilico_gating_scores[[i]][mask],
        pch = ifelse(cell_to_cluster[mask] == clust, 20, NA),
        col = 2
      )
      rect(
        xleft = get_min_umis(session),
        clustering_params$insilico_gating[[i]]$interval[1],
        get_max_umis(session),
        clustering_params$insilico_gating[[i]]$interval[2],
        lty = 3,
        lwd = 3,
        border = 2
      )
  ...
  output$genecor_heatmap_modules <- renderPlot({
    ...
    mtext(
      text = colnames(cormat)[ord],
      side = 1,
      at = seq(0, 1, l = ncol(cormat)),
      las = 2,
      cex = .5
    )
    mtext(
  ...
  output$samples_enrichment_plot <- renderPlot({
    ...
    tab = matrix(
      0,
      length(get_all_clusters(session)),
      length(get_loaded_samples(session))
    )
    rownames(tab) = get_all_clusters(session)
    colnames(tab) = get_loaded_samples(session)
    cells = select_cells(session, samples = insamples)
    tmptab = table(
      get_cell_to_cluster(session, cells = cells),
      get_cell_to_sample(session, cells = cells)
    )
  ...
  output$cluster_sizes_plot <- renderPlot({
    ...
    tmp_tab = table(get_cell_to_cluster(
      session,
      select_cells(session, samples = insamples)
    ))
  ...
  output$avg_heatmap_interactive <- renderPlotly({
    ...
    if (input$inModelOrAverage == "Model parameters") {
      ...
      arr_weights = array(
        weights,
        dim = c(length(insamples), length(ingenes), length(inclusts))
      )
      numis_per_clust_mat = apply(
        counts[insamples, , inclusts, drop = F],
        c(1, 3),
        sum,
        na.rm = T
      )
      numis_per_clust_arr = aperm(
        array(
          numis_per_clust_mat,
          dim = c(length(insamples), length(inclusts), length(ingenes))
        ),
        c(1, 3, 2)
      )
      ...
      if (input$inModelOrAverage == "Batch-corrected Average") {
        ...
        if (!is.null(noise_counts)) {
          ...
          noise_numis_per_clust_mat = apply(
            noise_counts,
            c(1, 3),
            sum,
            na.rm = T
          )
          noise_numis_per_clust_arr = aperm(
            array(
              noise_numis_per_clust_mat,
              dim = c(length(insamples), length(inclusts), length(ingenes))
            ),
            c(1, 3, 2)
          )
          numis_per_clust_arr = pmax(
            numis_per_clust_arr - noise_numis_per_clust_arr,
            0
          )
        }
      }

      mat = apply(counts, 2:3, sum, na.rm = T) /
        apply(numis_per_clust_arr, 2:3, sum, na.rm = T)
    ...
    clusters_text = paste(
      " (n=",
      session$userData$ncells_per_cluster[inclusts],
      " ; ",
      round(
        100 *
          session$userData$ncells_per_cluster[inclusts] /
          sum(session$userData$ncells_per_cluster[setdiff(
            names(session$userData$ncells_per_cluster),
            session$userData$scDissector_params$excluded_clusters
          )]),
        digits = 1
      ),
    ...
    return(plot_avg_heatmap_interactive(
      mat1,
      zlim,
      main_title,
      genes,
      ...
      clusters_text,
      annots,
      Relative_or_Absolute = abs_or_rel,
      colgrad = session$userData$modelColorGrad
    ))
  ...
  module_counts_reactive <- reactive({
    ...
    return(t(
      sapply(modsl, function(modi) {
        colSums(counts[modi, , drop = F])
      }) /
        totnumis
    ))
  })
  ...
  output$avg_heatmap_modules <- renderPlot({
    ...
    image(
      mat_to_show[, ncol(mat1):1],
      col = colgrad_rel,
      breaks = c(break1, seq(zlim[1], zlim[2], l = 99), break2),
      axes = F,
      main = session$userData$loaded_model_version
    )
    ...
    mtext(
      text = paste(
        ...
        round(
          100 *
            session$userData$ncells_per_cluster[inclusts] /
            sum(session$userData$ncells_per_cluster),
          digits = 1
        ),
        "% )",
        sep = ""
      ...
      side = 4,
      at = seq(1, 0, l = dim(mat1)[2]),
      las = 2,
      cex = 1
    )
    mtext(
      text = paste(cluster_annots_reactive()[inclusts], " ", sep = ""),
      side = 2,
      at = seq(1, 0, l = dim(mat1)[2]),
      las = 2,
      cex = 1
  ...
  output$avg_heatmap_samples <- renderPlot({
    ...
    isolate({
      ...
      image(
        mat_to_show[, ncol(mat1):1, drop = F],
        col = colgrad,
        breaks = c(break1, seq(zlim[1], zlim[2], l = 99), break2),
        axes = F,
        main = paste("Pooled Sample Averages over", clusters_string)
      )
    })
    box()
    ...
    mtext(
      ...
      las = 2,
      cex = 1,
      col = session$userData$gcol[toupper(rownames(mat1))]
    )
    mtext(
      text = colnames(mat1),
      side = 2,
  ...
  output$colorLegendModel <- renderPlot({
    ...
    image(
      matrix(1:100, 100, 1),
      breaks = 0:100,
      col = session$userData$modelColorGrad,
      axes = F
    )
    axs = c(
      input$inModelColorScale[1],
      round(mean(input$inModelColorScale), digits = 2),
      input$inModelColorScale[2]
    )
  ...
  output$downloadTruthPlot <- downloadHandler(
    filename = function() {
      paste(session$userData$loaded_model_version, '.png', sep = '')
    },
    content = function(file) {
      png(file, 1200, 900)
      plot_truth_heatmap_wrapper()
  ...
  plot_truth_heatmap_wrapper = function() {
    ...
    if (input$inTruthEqualRepresent == "equally_represent_samples") {
      cells_selected = sample_cells_by_sample_reactive()[[match(
        input$inTruthDownSamplingVersion,
        get_ds_options(session)
      )]][[input$inTruthNcellsPer]]
    } else {
      cells_selected = sample_cells_by_cluster_reactive()[[match(
        input$inTruthDownSamplingVersion,
        get_ds_options(session)
      )]][[input$inTruthNcellsPer]]
    }
    ...
    plot_truth_heatmap(
      ...
      showSeparatorBars = input$inTruthShowSeparatorBars,
      score_genes = score_genes,
      reverse_score_order = reverse_score_order,
  ...
  varmean_reactive <- reactive({
    ...
    ds = get_dstab(
      session,
      cells = select_cells(session, cluster = clust, samples = samples),
      ds_version = match(input$inQCDownSamplingVersion, get_ds_options(session))
    )

    if (is.null(ds)) {
      (return(data.frame(m = 0, varmean = 0, gene = 0)))
    }
    if (ncol(ds) < 2) {
      (return(data.frame(m = 0, varmean = 0, gene = 0)))
    }
    ...
    df = data.frame(
      m = logm[mask1],
      varmean = log2(v / m[mask1]),
      gene = rownames(ds)[mask1]
    )
  ...
  output$subtype_freqs_barplots <- renderPlot({
    ...
    celltypes = intersect(
      cluster_set_names,
      names(freq_norm)[(!sapply(freq_norm, is.null))]
    )
  ...
  vis <- reactive({
    ...
    varmean_reactive %>%
      ...
      add_axis(
        ...
        orient = "top",
        ticks = 0,
        title = paste(session$userData$loaded_model_version, clust),
  ...
  output$cellcor <- renderPlot({
    ...
    if (ncol(ds) < 2) {
      message(
        "Warning: Cluster ",
        clust,
        " has less than 2 cells above the UMIs threshold."
      )
      return()
    }
    ...
    var_genes = head(
      genemask[order(v[genemask] / m[genemask], decreasing = T)],
      200
    )
    z = ds[var_genes, ]
    z = z / mean(z)
    ...
    par(mar = c(.5, .5, 2, .5))
    image(
      t(as.matrix(match(samps, insamples))),
      axes = F,
      breaks = 0:length(insamples) + .5,
      col = sample_cols[1:length(insamples)]
    )
  ...
  output$gene2 <- renderText({
    paste("_________", input$inGene2, sep = "")
  })
  ...
  output$table <- renderTable(
    {
      ...
      ds = get_dstab(
        session,
        cells = select_cells(session, clusters = clust, samples = insamples),
        ds_version = match(
          input$inQCDownSamplingVersion,
          get_ds_options(session)
        )
      ...
      if (flag) {
        dat = floor(log2(
          1 + data.frame(t(as.matrix(ds[c(input$inGene1, input$inGene2), ])))
        ))
        tab = table(dat)
        x = matrix(
          0,
