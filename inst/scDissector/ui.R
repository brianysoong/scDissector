  ...
  fluidPage(
    ...
    navbarPage(
      ...
      tabPanel(
        "MetaData",
        fluidRow(
          DT::dataTableOutput("mytable", height = 600)
        ),
        fluidRow(
      ...
      tabPanel(
        ...
        fluidRow(
          ...
          column(
            ...
            offset = 0,
            style = 'padding:0px;',
            uiOutput("samples_enrichment")
          ),
          column(10, uiOutput("avg_profile_plot")),
          column(
            1,
            offset = 0,
            style = 'padding:0px;',
            uiOutput("cluster_sizes")
          ),
          ...
          column(
            ...
            wellPanel(
              ...
              selectInput(
                "inReorderingMethod",
                "Reordering method",
                choices = c("Hierarchical clustering", "Diagonal")
              )
            ),
            wellPanel(
              h4("Gene Selection"),
              ...
              selectInput(
                "inNgenes",
                "N=",
                choices = c(50, 100, 200, 300, 400, 500, 1000),
                width = 100
              ),
              actionButton(
                "inBlindChisqSelectedClusters",
                "Chi sq. screen on selected clusters"
              ),
              actionButton(
                "inVarMeanScreenSelectedClusters",
                "VarMean screen on selected clusters"
              ),
              sliderInput(
                "inMinExprForScreen",
                "Min expression (log10)",
                min = -7,
                max = -1,
                step = .1,
                value = c(-5, -1)
          ...
          column(
            ...
            wellPanel(
              ...
              textInput(
                "inOrderClusetersByGenes",
                "Order by gene formula:",
                width = 2000
              ),
            ...
            wellPanel(
              ...
              fluidRow(
                column(
                  6,
                  textInput("inAddClusterSet", "Type in name", value = "")
                ),
                column(6, h2(""), actionButton("inAddClusterSetButton", "Add"))
              ),
              fluidRow(
                column(
                  6,
                ...
                column(
                  6,
                  h2(""),
                  actionButton("inRemoveClusterSetButton", "Remove")
                )
              ),
        ...
        fluidRow(
          ...
          wellPanel(
            textInput(
              "inGenesScore",
              width = 2000,
              "Order cells within clusters by the following genes:"
            ),
            checkboxInput(
              "inScoreReverseOrder",
              label = "Reverse Order",
          ...
          selectInput(
            ...
            label = "Down-sampling version:",
            choices = c(""),
            width = 200
          ),
          checkboxInput(
            "inTruthShowSeparatorBars",
            label = "Show Separator Bars",
            value = T
          ),
          wellPanel(
            radioButtons(
              "inTruthEqualRepresent",
              "Equally represent:",
              c(
                "Samples" = "equally_represent_samples",
        ...
        column(
          ...
          wellPanel(
            ...
            textAreaInput(
              ...
              label = "ClusterSets to view:",
              value = ""
            ),
            actionButton("inReorderSamples", "Reorder samples")
          )
        ),
        column(12, uiOutput("subtype_freqs"), hr()),
      ...
      tabPanel(
        ...
        fluidRow(
          wellPanel(
            ...
            textInput(
              "inVarMean_varmeanThresh",
              "Min Log2(var/mean):",
              value = "0.50"
            )
          ),
          wellPanel(
            actionButton("inModulesGetCormap", "Get Correlation Map"),
            uiOutput("cor_gene_module_plot")
          ),
          wellPanel(
            textInput(
              "inNUmberOfGeneModules",
              "Number of Modules:",
              value = 50
            ),
          ...
          sliderInput(
            "inAvgModuleColorScale",
            "Log2(expression/(Module_mean))",
            min = -4,
            max = 4,
            step = 1,
            value = c(-2, 2)
