params = list()
params$nrandom_cells_per_sample_choices = c(
  100,
  250,
  500,
  1000,
...
surface_markers_file = system.file(
  "extdata",
  "Martinez_Martin_cell_2018.csv",
  package = "scDissector"
)
...
if (file.exists(surface_markers_file)) {
  surface_markers <- unique(read.csv(
    surface_markers_file,
    skip = 2,
    header = F,
    stringsAsFactors = F
  )[, 1])
  surface_markers2 = paste(
    substring(surface_markers, 1, 1),
    tolower(substring(surface_markers, 2)),
    sep = ""
  )
...
colgrad_rel_file = system.file(
  "extdata",
  "colors_brewer_RdBu.txt",
  package = "scDissector"
)
if (colgrad_rel_file == "") {
...
default_sample_colors_file = system.file(
  "extdata",
  "sample_colors.txt",
  package = "scDissector"
)
...
default_sample_colors <<- rep(
  paste(
    "#",
    read.table(default_sample_colors_file, stringsAsFactors = F)[, 1],
    sep = ""
  ),
  10
)
