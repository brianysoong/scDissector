...
plot_avg_heatmap = function(
  ...
  mtext(
    text = paste(" ", clusters, clusters_text),
    side = 4,
    at = seq(1, 0, l = length(clusters)),
    las = 2,
    cex = cex.clusters
  )
  mtext(
    text = paste(annots, " ", sep = ""),
    side = 2,
    at = seq(1, 0, l = length(annots)),
    las = 2,
    cex = cex.clusters
  )
}

plot_avg_heatmap_interactive = function(
  m,
  zlim,
  main_title,
  genes,
  ...
  heatmaply(
    ...
    labRow = paste(annots, " (", clusters_vec, ")", sep = ""),
    labCol = rownames(m),
    main_title = main_title,
    column_text_angle = 90,
    margins = c(100, 180, 20, 0),
    fontsize_row = 8,
    fontsize_col = 8
  )
}
...
plot_truth_heatmap = function(
  ...
  if ((!is.null(score_genes)) & (length(score_genes) >= 1)) {
    score_rank = rank(apply(
      log2(.1 + as.matrix(ds[score_genes, , drop = F])),
      2,
      mean
    )) /
      ncol(ds)
    ...
    cell_ord = order(
      (1 - global_order) *
        2 *
        match(cell_to_cluster[colnames(ds)], inclusts) +
        score_rank
    )
    if (global_order) {
      showSeparatorBars = F
    ...
    cell_ord = order(
      match(cell_to_sample[colnames(ds)], insamples) +
        length(insamples) * match(cell_to_cluster[colnames(ds)], inclusts)
    )
  }
  ...
  tmp_ncells = sapply(
    split(colnames(ds), cell_to_cluster[colnames(ds)]),
    length
  )
  ...
  image(
    ...
    col = c("gray", cols),
    axes = F,
    breaks = c(-3e6, -1e6, seq(zlim[1], zlim[2], l = 99), 1e6)
  )

  box()
  if (showSeparatorBars) {
    abline(
      h = 1 - cumsum(ncells) / sum(ncells),
      col = "gray",
      lwd = seperatorBars_lwd
    )
  }
  mtext(
    text = rownames(pmat),
    side = 1,
    at = seq(0, 1, l = dim(pmat)[1]),
    las = 2,
    cex = gene_text_cex,
    adj = 1,
    line = 1
  ...
  if (showClusterLabels) {
    mtext(
      text = inclusts,
      side = 2,
      at = 1 - (b / a[length(ncells)]),
      las = 2,
      cex = cluster_text_cex,
      adj = 1,
      line = .2
    )
  }
  if (plot_batch_bar) {
    ...
    image(
      t(as.matrix(match(rev(samps), insamples))),
      axes = F,
