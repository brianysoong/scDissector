...
#' @export
#
load_scDissector_data = function(
  ...
  sample_to_fn = read.csv(
    paste(clustering_data_path, "/samples.csv", sep = ""),
    stringsAsFactors = F,
    row.names = 1
  )
  ...
  model_fn = paste(
    ...
    "/",
    read.csv(
      paste(clustering_data_path, "/model_versions.csv", sep = ""),
      stringsAsFactors = F,
      row.names = 1
    )[model_name, 1],
    sep = ""
  ...
  sample_fns = paste(
    clustering_data_path,
    sample_to_fn[as.character(sample_names), 1],
    sep = "/"
  )
  ...
  ldm = load_dataset_and_model(
    ...
    max_umis = max_umis,
    ds_numis = ds_numis,
    max_ncells_per_sample = max_ncells_per_sample,
    genes = genes
  )
  return(ldm)
}
