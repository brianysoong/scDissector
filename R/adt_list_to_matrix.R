...
adt_list_to_matrix <- function(adt_list) {
  ...
  for (iter in 1:length(adt_list)) {
    ...
    adt_list[[iter]] <- adt_list[[iter]][
      !rownames(adt_list[[iter]]) %in% bad_rows,
    ]

    #remove ".IH" suffix
    rownames(adt_list[[iter]]) <- sub(
      x = rownames(adt_list[[iter]]),
      pattern = ".IH",
      replacement = ""
    )
    ...
    if (substr(rownames(adt_list[[iter]]), 1, 1) == 5) {
      rownames(adt_list[[iter]]) <- substr(
        rownames(adt_list[[iter]]),
        2,
        nchar(rownames(adt_list[[iter]]))
      )
    }
  }
