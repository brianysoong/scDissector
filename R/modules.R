...
aggregate_sparse = function(mat, v) {
  ...
  aggmat = Matrix(,
    length(v_unique),
    ncol(mat),
    dimnames = list(v_unique, colnames(mat))
  )
...
get_avg_gene_to_gene_cor = function(
  ds,
  cell_to_sample,
  samples = NULL,
  weighted = F,
  min_number_of_cell_per_sample = 5,
  min_umi_counts_per_samples = 5,
  showShinyProgressBar = F,
  session = NULL
) {
  zmat = matrix(
    0,
    nrow(ds),
    nrow(ds),
    dimnames = list(rownames(ds), rownames(ds))
  )
  ...
  samples = samples[
    table(cell_to_sample)[samples] >= min_number_of_cell_per_sample
  ]
  ...
  get_cor_per_sample = function() {
    for (samp in samples) {
      ...
      dsi = ds[
        Matrix::rowSums(ds[, cell_to_sample == samp, drop = F], na.rm = T) >
          min_umi_counts_per_samples,
        cell_to_sample == samp,
        drop = F
      ]
      ...
      z[is.na(z)] = 0
      #   print(range(z))
      zmat[rownames(z), colnames(z)] = zmat[rownames(z), colnames(z)] +
        w[samp] * z
      rm(z)
      gc()
    }
...
get_avg_module_to_gene_cor = function(
  ds,
  genes,
  modules_list,
  cell_to_sample,
  ...
  zmat = matrix(
    ...
    length(modules_list),
    length(genes),
    dimnames = list(names(modules_list), genes)
  )
  samples_inp = unique(cell_to_sample)
  if (is.null(samples)) {
    samples = samples_inp
  ...
  for (samp in samples) {
    print(samp)
    genes = genes[
      Matrix::rowSums(ds[genes, cell_to_sample == samp, drop = F], na.rm = T) >=
        min_umi_counts_per_samples
    ]
    dsi = log2(1 + ds[genes, cell_to_sample == samp, drop = F])

    get_one_mod_sum = function(x, ds) {
      colSums(log2(1 + as.matrix(ds[x, , drop = F])))
    }
    ds_mods_i = t(sapply(
      modules_list,
      get_one_mod_sum,
      ds[, cell_to_sample == samp, drop = F]
    ))
    z = fisher.r2z(
      .99 * cor(as.matrix(Matrix::t(ds_mods_i)), as.matrix(Matrix::t(dsi)))
    )
    #    z=fisher.r2z(.99*sparse.cor(Matrix::t(dsi)))
    z[is.na(z)] = 0
    #   print(range(z))
    zmat[rownames(z), colnames(z)] = z +
      w[samp] * zmat[rownames(z), colnames(z), drop = F]
    rm(z)
...
save_gene_cor_map = function(
  cormat,
  modules_version,
  zbreaks = c(-1, seq(-.5, .5, l = 99), 1),
  ser_method = "GW",
  cor_cols = colorRampPalette(c("blue", "white", "red"))(100)
) {
  pdf(
    paste(modules_version, "gene_cor.pdf", sep = "_"),
    width = ncol(cormat) / 12,
    height = ncol(cormat) / 12
  )
...
gene_cor_analysis = function(
  ldm,
  ds_version,
  min_varmean_per_gene = 0.15,
  min_number_of_UMIs = 50,
  genes_to_exclude = c(),
  clusters = NULL,
  samples = NULL,
  weighted = F,
  ...
  b = b[mask_llv]
  lo = loess(z ~ b)
  high_var_genes = names(which(
    (lv - predict(lo, newdata = x)) > min_varmean_per_gene
  ))
  message(length(high_var_genes), " High var genes")
  if (is.null(modules_list)) {
    cormat = get_avg_gene_to_gene_cor(
      log2(1 + ds[high_var_genes, ]),
      ldm$dataset$cell_to_sample[colnames(ds)],
      samples = samples,
      weighted = weighted
    )
  } else {
    cormat = get_avg_module_to_gene_cor(
      ds,
...
parse_modules = function(
  ...
  min_mod_cor = 0.1,
  zlim = c(-.9, .9),
  ord_viz_method = "OLO_complete",
  figures_path = "",
  tables_path = ""
) {
  gene_mask2 = names(which(
    apply(cormat, 1, quantile, 1 - mod_size / ncol(cormat), na.rm = T) >=
      min_mod_cor
  ))
  ...
  pdf(
    paste(figures_path, "module_cor_", modules_version, ".pdf", sep = ""),
    width = nmods / 10,
    height = nmods / 10
  )
  ...
  write.table(
    file = paste(tables_path, modules_version, "_modules.txt", sep = ""),
    sapply(modsl, paste, collapse = ","),
    row.names = T,
    col.names = F,
    quote = F
  )
...
example = function() {
  genes_to_exclude = c(
    grep("RP", rownames(ldm$dataset$umitab), val = T),
    grep("MT-", rownames(ldm$dataset$umitab), val = T)
  )
  #cormat=gene_cor_analysis(ldm,"2000",0.1,50,genes_to_exclude=genes_to_exclude)
  cormat = gene_cor_analysis(
    ldm,
    "2000",
    .5,
    50,
  ...
  pdf(
    paste(modules_version, "gene_cor.png", sep = "_"),
    width = ncol(cormat) / 12,
    height = ncol(cormat) / 12
  )
  #  ord=hclust(as.dist(1-cormat))$order
