...
#' @return LDM object
#' @export
load_dataset_and_model <- function(
  model_fn,
  sample_fns,
  min_umis = 250,
  model_version_name = "",
  max_umis = 25000,
  excluded_clusters = NA,
  ds_numis = NA,
  genes = NULL,
  max_ncells_per_sample = NA,
  lightweight = F,
  cell_list = NULL,
  ...
  if (is.null(names(sample_fns))) {
    names(sample_fns) = sapply(
      strsplit(sapply(strsplit(sample_fns, "/"), tail, 1), "\\_|\\."),
      function(x) {
  ...
  if (file.exists(cluster_sets_fn)) {
    ...
    output$cluster_sets <- get_cluster_set_tree(data.frame(
      node = nodes,
      parent = parents
    ))
  }
  ...
  if (is.null(model$avg_numis_per_model)) {
    model$avg_numis_per_model = rep(
      mean(Matrix::colSums(model$umitab)),
      ncol(model$models)
    )
    names(model$avg_numis_per_model) = colnames(model$models)
    tmptab = sapply(
      split(
        Matrix::colSums(model$umitab),
        model$cell_to_cluster[colnames(model$umitab)]
      ),
      mean
    )
  ...
  dataset$avg_numis_per_sample_model <- matrix(
    NA,
    length(samples),
    ncol(model$models),
    dimnames = list(samples, colnames(model$models))
  )
  ...
  dataset$noise_models = matrix(
    0,
    length(genes),
    length(samples),
    dimnames = list(genes, samples)
  )
  dataset$counts <- array(
    0,
    dim = c(length(samples), nrow(model$models), ncol(model$models)),
    dimnames = list(samples, rownames(model$models), colnames(model$models))
  )
  ...
  for (sampi in samples) {
    ...
    gc()
    tmp_env$umitab = tmp_env$umitab[, setdiff(
      colnames(tmp_env$umitab),
      tmp_env$noise_barcodes
    )]
    if (!is.null(cell_list)) {
      tmp_env$umitab = tmp_env$umitab[, intersect(
        colnames(tmp_env$umitab),
        cell_list[[sampi]]
      )]
    }
    ...
    if (
      (!is.na(max_ncells_per_sample)) &
        ncol(tmp_env$umitab) > max_ncells_per_sample
    ) {
      tmp_env$umitab = tmp_env$umitab[, sample(
        colnames(tmp_env$umitab),
        size = max_ncells_per_sample,
        replace = F
      )]
    }
    ...
    if (length(ds_numis) > 0) {
      if (!is.na(ds_numis)) {
        ds_numis_sampi = intersect(
          as.character(ds_numis),
          as.character(tmp_env$ds_numis)
        )
      } else {
      ...
      for (ds_i in ds_numis_sampi) {
        if (!is.null(cell_list)) {
          ...
          tmp_env$ds[[match(ds_i, tmp_env$ds_numis)]] = tmp_env$ds[[match(
            ds_i,
            tmp_env$ds_numis
          )]][, intersect(
            cell_list[[sampi]],
            setdiff(
              colnames(tmp_env$ds[[match(ds_i, tmp_env$ds_numis)]]),
              tmp_env$noise_barcodes
            )
          )]
          #
        ...
        # Bug fix AL 2/24/20 loading specified ds_numis
        colnames(tmp_env$ds[[match(ds_i, tmp_env$ds_numis)]]) = paste(
          sampi,
          colnames(tmp_env$ds[[match(ds_i, tmp_env$ds_numis)]]),
          sep = "_"
        )
      ...
      if (is.null(dataset$ds_numis)) {
        ...
        if (!all(dataset$ds_numis == ds_numis_sampi)) {
          message(
            "Warning! Some of the samples don't share the same downsampling UMIs values"
          )
    ...
    if (is.null(model$params$insilico_gating)) {
      ...
      if (!lightweight) {
        for (score_i in names(model$params$insilico_gating)) {
          dataset$insilico_gating_scores[[score_i]] = c(
            dataset$insilico_gating_scores[[score_i]],
            is_res$scores[[score_i]]
          )
          if (is.null(dataset$gated_out_umitabs[[score_i]])) {
            dataset$gated_out_umitabs[[score_i]] = list()
          }
          dataset$gated_out_umitabs[[score_i]][[
            sampi
          ]] = is_res$gated_out_umitabs[[score_i]]
        }
      }
      rm("is_res")
    }
    ...
    if (!is.null(tmp_env$adttab)) {
      colnames(tmp_env$adttab) <- paste(
        sampi,
        colnames(tmp_env$adttab),
        sep = "_"
      )
    ...
    if (!is.null(tmp_env$htotab)) {
      colnames(tmp_env$htotab) <- paste(
        sampi,
        colnames(tmp_env$htotab),
        sep = "_"
      )
      dataset$hto_by_sample[[sampi]] = tmp_env$htottab
    }

    barcode_mask = tmp_env$numis_before_filtering[colnames(umitab)] > min_umis &
      tmp_env$numis_before_filtering[colnames(umitab)] < max_umis
    ...
    if (is.null(model$alpha_noise) & is.null(model$avg_numis_per_model)) {
      ...
      if (!is.null(model$alpha_noise)) {
        ...
        if (is.na(fixed_noise[sampi])) {
          alpha_b = update_alpha_single_batch(
            umitab[projection_genemask, ],
            models,
            noise_model,
            reg = model$params$reg,
        ...
        res_l = getOneBatchCorrectedLikelihood(
          umitab = umitab[projection_genemask, ],
          cbind(models, noise_model),
          noise_model,
          alpha_noise = alpha_b,
          reg = model$params$reg
        )
        ...
        if (!is.null(model$avg_numis_per_model)) {
          ...
          gobclle_res = noiseEMsingleBatch(
            umitab = umitab[projection_genemask, ],
            models = models,
            noise_model = noise_model,
            avg_numis_per_model = avg_numis_per_model,
            reg = model$params$reg,
            max_noise_fraction = .75
          )
          ll = gobclle_res$ll
          dataset$beta_noise[sampi] = gobclle_res$beta_noise
          dataset$avg_numis_per_sample_model[
            sampi,
            names(gobclle_res$avg_numis_per_model)
          ] = gobclle_res$avg_numis_per_model
        }
      }

      cell_to_cluster = MAP(ll)
      cells_to_include = names(cell_to_cluster)[
        !cell_to_cluster %in% excluded_clusters
      ]
      ...
      for (cluster in included_clusters) {
        dataset$counts[sampi, genes, cluster] = Matrix::rowSums(umitab[
          genes,
          cell_to_cluster == cluster,
          drop = F
        ])
      }
      ...
      if (exists("ds_numis_sampi")) {
        for (ds_i in 1:length(ds_numis_sampi)) {
          ds_l[[ds_i]][[sampi]] <- tmp_env$ds[[match(
            ds_numis_sampi[ds_i],
            tmp_env$ds_numis
          )]][
            genes,
            intersect(
              colnames(tmp_env$ds[[match(
                ds_numis_sampi[ds_i],
                tmp_env$ds_numis
              )]]),
              cells_to_include
  ...
  if (length(dataset$ds_numis) > 0) {
    for (ds_i in 1:length(ds_numis_sampi)) {
      dataset$ds[[ds_i]] <- Matrix(,
        length(genes),
        ,
        dimnames = list(genes, NA)
      )
    }
  }
  for (sampi in samples) {
  ...
  if (!is.null(model$noise_models)) {
    dataset$noise_counts = get_expected_noise_UMI_counts_alpha(
      dataset$umitab,
      dataset$cell_to_cluster,
      dataset$cell_to_sample,
      dataset$noise_models,
      dataset$alpha_noise,
      colnames(model$models)
    )
  }
...
downsample = function(u, min_umis, chunk_size = 100) {
  ...
  breaks = unique(c(
    seq(from = 1, to = length(cell_mask), by = chunk_size),
    length(cell_mask) + 1
  ))
  ds = Matrix(
    0,
    nrow = nrow(u),
    length(cell_mask),
    dimnames = list(rownames(u), cell_mask)
  )

  for (i in 1:(length(breaks) - 1)) {
    ds[non_zero_mask, breaks[i]:(breaks[i + 1] - 1), drop = F] = Matrix(apply(
      u[non_zero_mask, cell_mask[breaks[i]:(breaks[i + 1] - 1)], drop = F],
      2,
      downsamp_one,
      min_umis
    ))
...
#' @return LDM object
#' @export
import_dataset_and_model <- function(
  model_version_name,
  ...
  barcode_mask = dataset$numis_before_filtering[colnames(umitab)] > min_umis &
    dataset$numis_before_filtering[colnames(umitab)] < max_umis
  dataset$min_umis = min_umis
  dataset$max_umis = max_umis
  umitab = umitab[, barcode_mask]
  ...
  if (is.null(insilico_gating)) {
    ...
    for (score_i in names(insilico_gating)) {
      ...
      dataset$gated_out_umitabs[[score_i]][[sampi]] = is_res$gated_out_umitabs[[
        score_i
      ]]
    }
  }
  ...
  for (sampi in samples) {
    ...
    tmp_counts = as.matrix(Matrix::t(
      fac2sparse(cell_to_cluster[maski]) %*% Matrix::t(umitab[, maski])
    ))
    dataset$counts[
      sampi,
      rownames(tmp_counts),
  ...
  order_fn = paste(fn_prefix, "_order.txt", sep = "")
  if (file.exists(order_fn)) {
    output$cluster_order = as.character(read.table(
      file = order_fn,
      header = F
    )[, 1])
  } else {
...
merge_dataset_and_models <- function(
  model_version_name,
  clustering_data_path,
  ldm_rdfile1,
  ldm_rdfile2,
  ...
  mask1 = names(ldm1$dataset$cell_to_cluster)[
    ldm1$dataset$cell_to_cluster %in% clusters1
  ]
  mask2 = names(ldm2$dataset$cell_to_cluster)[
    ldm2$dataset$cell_to_cluster %in% clusters2
  ]

  cells = c(
    paste(prefix1, names(ldm1$dataset$cell_to_cluster[mask1]), sep = ""),
    paste(prefix2, names(ldm2$dataset$cell_to_cluster[mask2]), sep = "")
  )
  umitab = cbind(ldm1$dataset$umitab[, mask1], ldm2$dataset$umitab[, mask2])
  cell_to_cluster = c(
    paste(prefix1, ldm1$dataset$cell_to_cluster[mask1], sep = ""),
    paste(prefix2, ldm2$dataset$cell_to_cluster[mask2], sep = "")
  )
  if (add_prefix_to_sampleids) {
    cell_to_sample = c(
      paste(prefix1, ldm1$dataset$cell_to_sample[mask1], sep = ""),
      paste(prefix2, ldm2$dataset$cell_to_sample[mask2], sep = "")
    )
  } else {
    cell_to_sample = c(
      ldm1$dataset$cell_to_sample[mask1],
      ldm2$dataset$cell_to_sample[mask2]
    )
  }
  ...
  colnames(models) = c(
    paste(prefix1, clusters1, sep = ""),
    paste(prefix2, clusters2, sep = "")
  )
  output <- list()
  output$scDissector_params <- list()
...
load_metacell_clustering = function(
  mc_rda,
  mat_rda,
  clustering_data_path,
  name = "",
  ...
  cell_to_sample = as.character(
    attributes(mat$object)$cell_metadata$amp_batch_id
  )
  names(cell_to_sample) = rownames(attributes(mat$object)$cell_metadata)
  cells = intersect(
    intersect(names(cell_to_cluster), names(cell_to_sample)),
    colnames(umitab)
  )
  umitab = umitab[, cells]
...
create_clustering_data_dir = function(path, samples = c(), model_names = c()) {
  ...
  samples_tab = matrix(
    ...
    length(samples),
    3,
    dimnames = list(NULL, c("index", "path", "title"))
  )
  samples_tab[, 1] = samples
  model_versions_tab = matrix(
    "",
    length(model_names),
    2,
    dimnames = list(NULL, c("title", "path"))
  )
  model_versions_tab[, 1] = model_names
  sample_annots_tab = matrix(
    "",
    length(samples),
    15,
    dimnames = list(
  ...
  write.csv(
    file = paste(path, "/samples.csv", sep = ""),
    samples_tab,
    row.names = F
  )
  write.csv(
    file = paste(path, "/model_versions.csv", sep = ""),
    model_versions_tab,
    row.names = F
  )
  write.csv(
    file = paste(path, "/metadata/sample_annots.csv", sep = ""),
    sample_annots_tab,
    row.names = F
  )
}

transform_models = function(
  scDissector_datadir,
  model_fn,
  samples_fn,
  genes,
  ...
  init_noise = 0.1,
  min_umis = 300,
  ncells_per_sample = 500,
  reg = 1e-6
) {
  samples = names(samples_fn)
  fixed_noise = rep(init_noise, length(samples))
  names(fixed_noise) = samples
  ldm = load_dataset_and_model(
    model_fn,
    sample_fns = fns,
    model_version_name = model_fn,
    min_umis = min_umis,
    max_umis = 25000,
    ds_numis = c("1000"),
    lightweight = T,
    ...
    genes = genes,
    max_ncells_per_sample = ncells_per_sample
  )
  if (is.null(clusters)) {
  ...
  data_based_models = update_models_debatched(
    umis = ldm$dataset$umitab[genes, ],
    cell_to_cluster = ldm$dataset$cell_to_cluster,
    batch = ldm$dataset$cell_to_sample,
    noise_models = ldm$dataset$noise_models[genes, ],
    alpha_noise = ldm$dataset$alpha_noise,
    make_plots = F
  )
  clusters = intersect(colnames(data_based_models), clusters)
  data_based_models = data_based_models[, clusters]
  cor_vec = (mapply(clusters, FUN = function(i) {
    cor(log(reg + data_based_models[, i]), log(reg + init_models[genes, i]))
  }))
  print(cor_vec)
  #clust_mask=colnames(data_based_models)[cor_vec>cor_thresh]
  data_based_models_adj = t(
    t(data_based_models[genes, ]) / colSums(data_based_models[genes, ])
  )
  init_models_adj = t(t(init_models[genes, ]) / colSums(init_models[genes, ]))
  weights = 1e-10 +
    pmax(data_based_models_adj[, clusters], init_models_adj[, clusters])
  weights = weights / rowSums(weights)
  conversion_vector = rowSums(
    ((reg + data_based_models_adj[, clusters]) /
      (reg + init_models_adj[, clusters])) *
      weights
  )
  ...
  transformed_models_adj = t(
    t(transformed_models) / colSums(transformed_models)
  )
  model = ldm$model
