update_beta_single_batch = function(
  umitab,
  models,
  noise_model,
  avg_numis_per_model,
...
noiseEMsingleBatch = function(
  ...
  getOneBatchCorrectedLikelihood_beta = function(
    umitab,
    models,
    noise_model,
    beta_noise = NULL,
    ...
    ll[colnames(umitab), colnames(adjusted_models)] = getLikelihood(
      umitab,
      adjusted_models,
      reg = reg
    )
  ...
  beta_noise = update_beta_single_batch(
    umitab,
    models,
    noise_model,
    avg_numis_per_model,
  ...
  while (nmoved >= min_n_moved && i < 6) {
    res_boll = getOneBatchCorrectedLikelihood_beta(
      umitab,
      models = models,
      noise_model,
    ...
    tmptab = sapply(
      split((Matrix::colSums(umitab)), cell_to_cluster[colnames(umitab)]),
      mean
    )
